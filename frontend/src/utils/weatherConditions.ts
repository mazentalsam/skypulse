...
const CONDITIONS: Record<string, WeatherTheme> = {
...
const DEFAULT: WeatherTheme = { gradient: ['#060918', '#0a1128', '#0d1530'], label: 'Weather' };

export function getWeatherTheme(iconCode: string): WeatherTheme {
  return CONDITIONS[iconCode] || DEFAULT;
}

export function getWeatherIconUrl(iconCode: string, size = 2): string {
