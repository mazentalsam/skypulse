...
export default function AlertsPanel() {
  ...
  const [form, setForm] = useState<AlertForm>({ location: '', condition: 'temperature', threshold_value: '', threshold_type: 'greater_than', latitude: 0, longitude: 0 });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (err) { setError(typeof err === 'string' ? err : (err as string[]).join(', ')); return; }
