...
export default function AITripAdvisor() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error: err } = await fetchTripAdvice(destination, dates, { current: weatherRes.data?.weather, forecast: forecastData } as never);
    setLoading(false);
    if (err) { setError(err); return; }
    setAdvice(data as TripAdvice | null);
  };
        ...
        <div style={{ marginTop: 14 }}>
          ...
          {advice && (
            <div style={{ marginTop: 14, color: 'var(--muted)', fontSize: 13.5, lineHeight: 1.6 }}>
              {(advice.best_days?.length ?? 0) > 0 && <p><b style={{ color: 'var(--text)', fontWeight: 500 }}>Best days:</b> {advice.best_days!.join(' ')}</p>}
              {(advice.packing_list?.length ?? 0) > 0 && <p style={{ marginTop: 8 }}><b style={{ color: 'var(--text)', fontWeight: 500 }}>Pack:</b> {advice.packing_list!.join(' · ')}</p>}
              {(advice.activities_to_avoid?.length ?? 0) > 0 && <p style={{ marginTop: 8 }}><b style={{ color: 'var(--text)', fontWeight: 500 }}>Avoid:</b> {advice.activities_to_avoid!.join(' ')}</p>}
