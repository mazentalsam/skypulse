...
export default function PlanMyWeek() {
  ...
  const [events, setEvents] = useState<Array<{ event: string; day: string; time: string }>>([{ event: '', day: 'Monday', time: '09:00' }]);
  ...
  const removeEvent = (i: number) => setEvents(events.filter((_, j) => j !== i));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (data?.plans) setPlans(data.plans as WeekPlan[]);
  };
