...
export default function DateRangeSearch() {
  ...
  const pickCity = (city) => { setLocation(city); setSuggestions([]); };
      ...
      {result && (
        <div className="rise" style={{
          ...
          <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
            <div style={{ fontSize: 28, fontWeight: 500, letterSpacing: '-0.02em' }}>{formatTemp(result.weather_data?.main?.temp)}</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', textTransform: 'capitalize' }}>{result.weather_data?.weather?.[0]?.description || ''}</div>
          </div>
          <div style={{
