...
interface MediaData {
  source?: string;
  videos?: Array<{ video_id: string; thumbnail: string; title: string; channel: string }>;
  photos?: Array<{ url: string; description: string; photographer: string }>;
...
export default function YouTubePanel() {
  ...
  useEffect(() => {
    ...
    setMedia(null);
    fetchYouTubeVideos(location.city).then(({ data }) => setMedia(data as MediaData | null));
  }, [location?.city]);
  ...
  return (
    <div className="rise stagger-8" style={{
      ...
      <div style={{
        ...
        {isVideo ? (media.videos || []).slice(0, 6).map((v, i) => (
