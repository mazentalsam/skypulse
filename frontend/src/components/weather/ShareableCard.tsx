...
export default function ShareableCard() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.click();
    } catch { /* canvas generation may fail in some browsers */ }
    setGenerating(false);
  };
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
        <button onClick={handleDownload} disabled={generating} style={{
          ...
          onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => e.currentTarget.style.background = 'rgba(255,255,255,.04)'}
