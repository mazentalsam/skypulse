...
function getTempColor(temp: number): TempColor {
...
export default function ForecastRow() {
  ...
  const dailyMap: Record<string, { items: ForecastItem[]; min: number; max: number }> = {};
  ...
  const days = Object.entries(dailyMap).slice(0, 5).map(([, data]) => {
      ...
      {days.map((day, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = `0 8px 24px ${c.border}`; }}
