...
export default function SearchBar({ showHero = false }: SearchBarProps) {
  ...
  const handleGeo = async () => {
    ...
    } catch { /* geolocation may be denied */ }
  };
  ...
  const searchBox = (
    <div style={{ position: 'relative', maxWidth: 560, margin: '0 auto' }}>
      <div style={{
        ...
        <button onClick={(e: React.MouseEvent<HTMLButtonElement>) => handleSubmit(e)} disabled={loading || !query.trim()} type="button" style={{
        ...
        <div style={{
          ...
          {suggestions.map((city, i) => (
            <button key={city}
              ...
              onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = 'rgba(59,130,246,.08)'; e.currentTarget.style.color = 'var(--text)'; }}
              onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { if (i !== selectedIdx) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted)'; } }}
            >
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 18, flexWrap: 'wrap' }}>
        {QUICK.map(({ label }) => (
          <button key={label} onClick={() => doSearch(label)} style={{
            ...
            onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.color = 'var(--text)'; e.currentTarget.style.borderColor = 'var(--border-strong)'; e.currentTarget.style.background = 'rgba(255,255,255,.03)'; }}
            onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.color = 'var(--muted)'; e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'transparent'; }}
          >{label}</button>
        ))}
      </div>
    </div>
