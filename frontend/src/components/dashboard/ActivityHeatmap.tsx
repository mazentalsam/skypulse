          ...
          <div key={wi} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            {week.map((cell, di) => (
              <div key={di} title={`${cell.date}: ${cell.count} search${cell.count !== 1 ? 'es' : ''}`}
                ...
                onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => { if (cell.count) e.currentTarget.style.outline = '1px solid rgba(59,130,246,.4)'; }}
                onMouseLeave={(e: React.MouseEvent<HTMLDivElement>) => e.currentTarget.style.outline = 'none'}
              />
            ))}
          </div>
        ))}
