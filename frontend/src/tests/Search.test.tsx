...
vi.mock('../api/client', () => {
  ...
  return {
    ...
    getExportUrl: vi.fn((id: number, fmt: string) => `/api/export/${id}?format=${fmt}`),
    getExportAllUrl: vi.fn((fmt: string) => `/api/export/all?format=${fmt}`),
  };
});
...
describe('Search flow', () => {
  ...
  it('searches for a city and displays weather results', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/scattered clouds/i).length).toBeGreaterThan(0);
    });
  });
  ...
  it('displays error message when search fails', async () => {
    const { fetchCurrentWeather } = await import('../api/client');
    (fetchCurrentWeather as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      data: null,
